    ...
    <div className="min-h-screen bg-slate-50 py-12">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-12">
          ...
          <h1 className="text-3xl font-bold text-slate-900 mb-2">Chatbot Published Successfully!</h1>
          <p className="text-lg text-slate-600">Your chatbot is now ready to be integrated. Choose your preferred integration method below.</p>
        </div>
        ...
        <div className="space-y-8">
          ...
          <div className="bg-white shadow rounded-lg overflow-hidden">
            <div className="p-6">
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  <h3 className="text-lg font-semibold text-slate-900">Website Integration</h3>
                  <p className="text-sm text-slate-500">Add this code to your website's HTML</p>
                </div>
              </div>
              <div className="relative">
                <pre className="bg-slate-50 rounded-lg p-4 overflow-x-auto">
                  <code className="text-sm text-slate-800">{scriptCode}</code>
                </pre>
                <button
                  onClick={handleCopyCode}
                  className="absolute top-3 right-3 p-2 rounded-md hover:bg-slate-200 transition-colors"
                >
                  {copied ? (
              ...
              <div className="mt-6 border-t pt-6">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h4 className="font-medium text-slate-900">Connect with Shopify</h4>
                      <p className="text-sm text-slate-500">Integrate the chatbot directly with your Shopify store</p>
                    </div>
                  </div>
                  <button
          ...
          <div className="bg-white shadow rounded-lg overflow-hidden">
            <div className="p-6">
              <h3 className="text-lg font-semibold text-slate-900 mb-6">Additional Integration Options</h3>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {integrationOptions.slice(1).map((option, index) => (
                  <div
                    key={index}
                    className="flex flex-col items-center p-6 border rounded-lg hover:border-indigo-500 transition-colors cursor-pointer"
                    onClick={() => setSelectedOption(option.title)}
                  >
                    <div className="h-12 w-12 rounded-lg bg-indigo-100 flex items-center justify-center mb-4">
                      <option.icon className="h-6 w-6 text-indigo-600" />
                    </div>
                    <h4 className="text-base font-medium text-slate-900 text-center mb-2">{option.title}</h4>
                    <p className="text-sm text-slate-500 text-center">{option.description}</p>
                  </div>
                ))}
              </div>
            </div>
