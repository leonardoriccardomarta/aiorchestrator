        ...
        <div className="bg-white rounded-lg shadow-sm p-6 mb-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-slate-600 mt-1">Manage customer conversations in real-time</p>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mt-6">
            <div className="bg-gradient-to-br from-indigo-50 to-indigo-100 p-4 rounded-lg">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm text-indigo-600 font-medium">Active Chats</p>
                  <p className="text-2xl font-bold text-indigo-900">{stats.activeChats}</p>
                </div>
          ...
          <div className="bg-white rounded-lg shadow-sm p-6">
            ...
            <div className="space-y-3">
              {queueConversations.length === 0 ? (
                <p className="text-slate-500 text-center py-8">No conversations waiting</p>
              ) : (
                queueConversations.map((conv) => (
                  <div key={conv.id} className="border border-slate-200 rounded-lg p-4 hover:border-indigo-300 transition-all">
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        <p className="font-semibold text-slate-900">{conv.visitorName}</p>
                        <p className="text-sm text-slate-600">{conv.visitorEmail}</p>
              ...
              {selectedConversation.messages.map((msg) => (
                <div key={msg.id} className={`mb-3 ${msg.sender === 'agent' ? 'text-right' : ''}`}>
                  ...
                  <div className={`inline-block px-4 py-2 rounded-lg ${
                    msg.sender === 'agent' ? 'bg-indigo-600 text-white' : 'bg-slate-100 text-slate-900'
                  }`}>
            ...
            <div className="p-4 border-t">
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 px-3 py-2 bg-white text-slate-900 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 placeholder:text-slate-500"
                />
                <button
                  onClick={sendMessage}
