...
const Connections: React.FC = () => {
  ...
  const fetchConnections = async () => {
    try {
      ...
      const response = await fetch('http://localhost:4000/api/connections', {
        headers: {
  ...
  const realWooCommerceOAuthFlow = async () => {
    try {
      ...
      const response = await fetch('http://localhost:4000/api/woocommerce/connect', {
        method: 'POST',
        headers: {
  ...
  const realShopifyOAuthFlow = async () => {
    try {
      ...
      // Make real API call to connect
      const token = localStorage.getItem('authToken');
      const response = await fetch('http://localhost:4000/api/shopify/connect', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`,
